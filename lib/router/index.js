...
exports.default = function () {
  ...
  this.onReady().then(function () {
    ...
    .add('/activeRanking', 'get', function (opts, cb, next) {
      ...
      var utcOffset = data.utcOffset || service.config.utcOffset;
      var UTC = Number(utcOffset || 0);
      var mstartDate = (0, _moment2.default)();
      var mendDate = (0, _moment2.default)();
      if (startDate) {
        if (isNaN(Date.parse(startDate))) {
          return cb(null, t(Err.FA_PARAMS_FAIL, lng, { params: 'startDate' }));
        }
        mstartDate = (0, _moment2.default)(new Date(startDate));
        mendDate = (0, _moment2.default)(new Date(startDate));
      }
      startDate = mstartDate.subtract(1, 'months').startOf('months');
      var endDate = mendDate.endOf('months');

      var curDateStr = endDate.format('YYYY-MM');

      var conditions = { crtime: { $gte: startDate.toDate(), $lte: endDate.toDate() } };
      var q = service.goods.aggregate();
      q.match(conditions);
      q.project({
